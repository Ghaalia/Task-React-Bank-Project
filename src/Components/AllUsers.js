import React from "react";
import { getAllUsers } from "../api/profile";
import { useQuery } from "@tanstack/react-query";

const AllUsers = () => {
  const { data } = useQuery({
    queryKey: ["allUsers"],
    queryFn: () => getAllUsers(),
  });

  const userslist = data?.map((user) => {
    return (
      <div className="flex justify-evenly flex-col text-white ">
        <div className="flex justify-center relative h-[100px] items-center">
          <div className="px-4 relative flex items-center justify-center gap-20 border border-solid h-[100%] w-[80%]  border-gray-500 border-opacity-5 shadow-lg shadow-black rounded-md">
            <img
              className="h-[90%] left-20 absolute rounded-full"
              src="data:image/png;base64,"
            />

            <h1>{user.username}</h1>
            <h1> {user.balance} K.D.</h1>
            <button className=" border border-white border-solid hover:bg-gray-700 hover:text-green-600 rounded-md px-2 py-0">
              Transfer
            </button>
          </div>
        </div>
      </div>
    );
  });

  return <div>{userslist}</div>;
};

export default AllUsers;
